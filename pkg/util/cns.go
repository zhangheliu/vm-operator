...
// GetAttachmentsForVM returns the existing CnsNodeVmAttachments for the
// specified VM by filtering attachments that match the VM's BiosUUID.
//
...
func GetAttachmentsForVM(
	...
	err := k8sClient.List(ctx, list,
		...
		ctrlclient.MatchingFields{
			"spec.nodeuuid": vm.Status.BiosUUID,
		})
	if err != nil {
		return nil, fmt.Errorf(
			"failed to list CnsNodeVmAttachments: %w", err)
	}
